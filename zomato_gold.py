...
def execute_script(pageNum, city):
    ...
    page_css_elements = {}
    # Get scroll height
    document_height = bot.execute_script("return document.body.scrollHeight")
    for i in range(0, document_height, 1080):
        # Scroll down to bottom
        bot.execute_script("window.scrollTo(0, {});".format(i))
        # Wait to load page
        time.sleep(1)
    time.sleep(1)
    for name, cssSelector in REWARD_DETAILS_CSS_SELECTORS.items():
        try:
            page_css_elements[name] = (
                bot.find_elements(By.CSS_SELECTOR, cssSelector))
...
def sqlDeletions(sqlConn, REWARD_ORIGIN):
    if len(sqlConn.select_by_reward_origin(REWARD_ORIGIN)) > 0:
        sqlConn.delete_by_reward_origin(REWARD_ORIGIN)


def sqlInsertions(sqlConn, results, REWARD_ORIGIN, REWARD_ORIGIN_LOGO):
...
sqlDeletions(sqlConn, REWARD_ORIGIN)
for i in COUNTRY_LIST:
    results = []
    bot.get(URL_1+i+URL_2)
    time.sleep(1)

    try:
        bot.find_element(By.CSS_SELECTOR, NO_REWARDS_CSS_SELECTOR)
        print('no gold partners for {} in Zomato'.format(i))
    except NoSuchElementException:
        execute_script(1, i)

        numberOfPages = int(bot.find_element(
            By.CSS_SELECTOR, PAGE_NUMBER_CSS_SELECTOR).text.split(' ')[3])

        for j in range(numberOfPages-1):
            nextPage = bot.find_element(
                By.CSS_SELECTOR, NEXT_PAGE_BUTTON_CSS_SELECTOR)
            nextPage.click()
            time.sleep(1)
            execute_script(j+2, i)
    sqlInsertions(sqlConn, results, REWARD_ORIGIN, REWARD_ORIGIN_LOGO)
    print('{} - {}'.format(i, len(results)))
